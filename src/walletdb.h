...
// Copyright (c) 2016-2018 The LogisCoin developers
...
public:
    ...
    bool WriteZLGSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZLGSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZLGSSeed_deprecated(uint256& seed);
    bool EraseZLGSSeed();
    bool EraseZLGSSeed_deprecated();

    bool WriteZLGSCount(const uint32_t& nCount);
    bool ReadZLGSCount(uint32_t& nCount);
