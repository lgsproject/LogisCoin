...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 LGS at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zLGS send
enum ZerocoinSpendStatus {
    ZLGS_SPEND_OKAY = 0,                            // No error
    ZLGS_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZLGS_WALLET_LOCKED = 2,                         // Wallet was locked
    ZLGS_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZLGS_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZLGS_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZLGS_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZLGS_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZLGS_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZLGS_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZLGS_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZLGS_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZLGS_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZLGS_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZLGS_SPENT_USED_ZLGS = 14,                      // Coin has already been spend
    ZLGS_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZLGS_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZLGSOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzLGSWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzLGSTracker> zpivTracker;
    ...
    void setZWallet(CzLGSWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzLGSTracker>(new CzLGSTracker(strWalletFile));
    }

    CzLGSWallet* getZWallet() { return zwalletMain; }
    ...
    /** zLGS reset */
    boost::signals2::signal<void()> NotifyzLGSReset;
