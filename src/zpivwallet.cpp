...
CzLGSWallet::CzLGSWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZLGSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZLGSSeed_deprecated()) {
                    LogPrintf("%s: Updated zLGS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzLGSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZLGSCount(nCountLastUsed);
    else if (!walletdb.ReadZLGSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzLGSWallet::Lock()
{
    seedMaster = 0;
}

void CzLGSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzLGSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZLGS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzLGSWallet::LoadMintPoolFromDB()
{
...
void CzLGSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzLGSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzLGSWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzLGSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZLGS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzLGSWallet::SeedToZLGS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzLGSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzLGSWallet::GenerateDeterministicZLGS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzLGSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzLGSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
